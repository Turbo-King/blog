...
通过解释器模式来实现四则运算，如计算 a+b-c的值，具体要求如下：
...
3. 应用场景
    ...
    - 一个简单语法需要解释的场景
4. 这样的例子还有，比如**编译器**、**元算表达式计算**、**正则表达式**、**机器人**等

##### 解释器模式原理

![解释器模式原理类图](https://cdn.jsdelivr.net/gh/Turbo-King/images/%E8%A7%A3%E9%87%8A%E5%99%A8%E6%A8%A1%E5%BC%8F%E5%8E%9F%E7%90%86%E7%B1%BB%E5%9B%BE.png "解释器模式原理类图")

{{< admonition question 解释器模式中角色及职责 >}}

- **Context**：是**环境角色**，含有解释器之外的全局信息
...
![解释器模式解决四则运算实例类图](https://cdn.jsdelivr.net/gh/Turbo-King/images/%E8%A7%A3%E9%87%8A%E5%99%A8%E6%A8%A1%E5%BC%8F%E8%A7%A3%E5%86%B3%E5%9B%9B%E5%88%99%E8%BF%90%E7%AE%97%E5%AE%9E%E4%BE%8B%E7%B1%BB%E5%9B%BE.png "解释器模式解决四则运算实例类图")
...
![抽奖活动状态转换图](https://cdn.jsdelivr.net/gh/Turbo-King/images/%E6%8A%BD%E5%A5%96%E6%B4%BB%E5%8A%A8%E7%8A%B6%E6%80%81%E8%BD%AC%E6%8D%A2%E5%9B%BE.png "抽奖活动状态转换图")

#### 状态模式

##### 基本介绍

1. **状态模式（State Pattern）**：它主要用来解决对象在**多种状态转换**时，需要对外输出不同的行为的问题。**状态和行为是一一对应的**，状态之间可以相互转换
2. 当一个对象的内在状态改变时，允许改变其行为，这个对象看起来像是改变了其类

##### 状态模式原理类图

![状态模式原理类图](https://cdn.jsdelivr.net/gh/Turbo-King/images/%E7%8A%B6%E6%80%81%E6%A8%A1%E5%BC%8F%E5%8E%9F%E7%90%86%E7%B1%BB%E5%9B%BE.png "状态模式原理类图")

{{< admonition question 状态模式中角色及职责 >}}

- **Context** 类：**环境角色**，用于维护 State 实例，这个实例**定义当前状态**
- State：**抽象状态角色**，定义一个接口封装与 Context 的一个特点接口相关行为
- **ConcreteState**：**具体的状态角色**，每个子类实现一个与 Context 的一个**状态**相关行为
...
![状态模式解决App抽奖应用实例](https://cdn.jsdelivr.net/gh/Turbo-King/images/%E7%8A%B6%E6%80%81%E6%A8%A1%E5%BC%8F%E8%A7%A3%E5%86%B3App%E6%8A%BD%E5%A5%96%E5%BA%94%E7%94%A8%E5%AE%9E%E4%BE%8B.png "状态模式解决App抽奖应用实例")
...
这类代码难以应对变化，在添加一种状态时，我们需要手动 添加 if/else，在添加一种功能时，要对所有的状态进行判断。 因此代码会变得越来越臃肿，并且一旦没有处理某个状态， 便会发生极其严重的 **BUG**，难以维护
...
2. 方便维护。将容易产生问题的 if/else 语句删除了，如果把每个状态的行为都放到一个类中，每次调用方法时都要判断当前是什么状态，不但会产生很多 if/else 语句，而且容易出错
3. 符合“开闭原则”。容易增删状态
4. 会产生很多类。每个状态都要一个对应的类，当状态过多时会产生很多类，加大维护难度
5. 对应场景：当一个事件或者对象有很多种状态，状态之间会相互转换，对不同的状态要求有不同的行为的时候，可以考虑使用状态模式
...
1. 有各种鸭子（比如：野鸭、北京鸭、水鸭等，鸭子有各种行为，比如：叫声、飞行等）
2. 显示鸭子信息

#### 传统方法

![传统方案解决鸭禽问题](https://cdn.jsdelivr.net/gh/Turbo-King/images/%E4%BC%A0%E7%BB%9F%E6%96%B9%E6%A1%88%E8%A7%A3%E5%86%B3%E9%B8%AD%E7%A6%BD%E9%97%AE%E9%A2%98.png "传统方案解决鸭禽问题")
...
2. 上面所出现的问题，其实是继承带来的问题：对类的局部改动，尤其超类的局部改动会影响其他部分。会有溢出效应
3. 为了改进上述问题，我们可以通过覆盖 fly 方法来解决 -> 覆盖解决
4. 问题又来了，如果我们有个一个玩具鸭子 ToyDuck，这样就需要 ToyDuck 去覆盖 Duck 的所有实现的方法 -> 解决思路 **策略模式**（strategy pattern）
...
1. 策略模式（Strategy Pattern）:定义算法族，分别封装起来，让他们之间可以互相替换，此模式让算法的变化独立于使用算法的客户
2. 这算法体现了几个设计原则
    - 第一：把变化的代码从不变的代码分离出来；
    - 第二：针对接口编程而不是具体的类（定义了策略接口）；
    - 第三：多用组合/聚合，少用继承（客户通过组合方式使用策略）

##### 策略模式原理类图

![策略模式原理类图](https://cdn.jsdelivr.net/gh/Turbo-King/images/%E7%AD%96%E7%95%A5%E6%A8%A1%E5%BC%8F%E5%8E%9F%E7%90%86%E7%B1%BB%E5%9B%BE.png "策略模式原理类图")
...
![策略模式解决鸭禽问题实例类图](https://cdn.jsdelivr.net/gh/Turbo-King/images/%E7%AD%96%E7%95%A5%E6%A8%A1%E5%BC%8F%E8%A7%A3%E5%86%B3%E9%B8%AD%E7%A6%BD%E9%97%AE%E9%A2%98%E5%AE%9E%E4%BE%8B%E7%B1%BB%E5%9B%BE.png "策略模式解决鸭禽问题实例类图")
...
4. 提供了可以替换继承关系的办法：策略模式即将算法封装在独立的 Strategy 类中使得你可以独立于其 Context 改变它，使它易于切换、易于理解、易于扩展
